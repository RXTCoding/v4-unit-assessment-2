...
for (let i=0; i < classes.length; i++){ //<--for loops dont have to 
    for (let key in classes[i] ){
        if(classes[i][key] === true){ //<__brackets can live next to each other 
            classes[i][key] = false; //<--- whats the difference between this sntax and return false? is it because return only works in a function?
        }
    } 
}  
...
*/
  ...
  let phone1 = new Phone ('Nokia', 'gen X', 64, 'red', false)
  
  let phone2 = new Phone ('iPhone', 'X', 132, 'yellow', false)
  
  let phone3 = new Phone ('samsung', 'Galaxy 21', 282, 'purple', false)
